...
class FTP(Base):
	...
	def generate_packet(self, cmd: str, arg: Optional[str] = None) -> Request:
		if arg is None:
			...
			block = Block(
				name="Command",
				children=(String(name="command", default_value=cmd), Delim(name="space", default_value=" ") if arg else None, String(name="argument", default_value=arg) if arg else None),
			)
